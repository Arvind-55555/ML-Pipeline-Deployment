...
@click.option(
    "--symbols",
    multiple=True,
    help="Stock symbols to monitor (e.g., --symbols AAPL,GOOGL)",
)
...
def stocks(symbols, interval):
    ...
    if symbols:
        ...
        for sym in symbols:
            ...
            symbol_list.extend(
                [s.strip().upper() for s in str(sym).split(",") if s.strip()]
            )
        pipeline.config["symbols"] = (
            symbol_list if symbol_list else [s.upper() for s in symbols]
        )
...
def start_realtime(config):
    ...
    continuous_training.start_continuous_training(
        data_fetcher=fetch_latest_data, validation_data=load_validation_data()
    )
