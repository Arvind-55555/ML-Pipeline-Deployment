...
class RecommendationPipeline:
    ...
    def __init__(self):
        ...
        self.config = {"top_k": 10, "min_interactions": 5}
    ...
    def update_feature_store(self):
        ...
        if interactions:
            ...
            logger.info(
                f"✅ Feature store updated: {len(self.user_item_matrix)} users, {len(self.user_item_matrix.columns)} items"
            )
        else:
    ...
    def get_popular_items(self, top_k: int = 10) -> List[Dict[str, Any]]:
        ...
        for item_id, features in self.feature_store["item_features"].items():
            ...
            items.append(
                {"item_id": item_id, "score": popularity_score, "features": features}
            )
